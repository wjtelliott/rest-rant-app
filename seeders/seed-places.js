const db = require('../models');
const mongoose = require('mongoose');
require('dotenv').config();

mongoose.connect(process.env.DB_URI, { useNewUrlParser: true, useUnifiedTopology: true })
    .then(() => console.log('Connected to MongoDB'))
    .catch(err => console.log('Connection to MongoDB failed!', err));

db.Place.insertMany([{
    name: 'Maui',
    city: 'My City',
    state: 'My State',
    cuisines: 'BeeseChurger',
    pic: 'https://imagesvc.meredithcorp.io/v3/mm/image?url=https%3A%2F%2Fstatic.onecms.io%2Fwp-content%2Fuploads%2Fsites%2F28%2F2021%2F02%2F23%2Fmaui-hawaii-MAUITG0221.jpg&w=1100&h=737&c=sc&poi=face&q=60',
    uid: 0,
    archived: false
}, {
    name: 'Maui, Again',
    city: 'My City2',
    state: 'My State2',
    cuisines: 'BeeseChurger2',
    pic: 'https://a.cdn-hotels.com/gdcs/production54/d1246/2a4051b0-2b61-4d80-8687-880705a77600.jpg?impolicy=fcrop&w=1600&h=1066&q=medium',
    uid: 1,
    archived: true
}, {
    name: 'Hawaii',
    city: 'Hawaii City',
    state: 'Hawaii State',
    cuisines: 'BeeseChurgers',
    pic: 'https://www.gohawaii.com/sites/default/files/styles/image_gallery_bg_xl/public/hero-unit-images/10316%202.jpg?itok=l-iF3eE0',
    uid: 2,
    archived: false
}, {
    name: 'Hawaii Horse Trail',
    city: 'Trail City',
    state: 'Trail State',
    cuisines: 'BeeseChurgers yet again',
    pic: 'https://www.gohawaii.com/sites/default/files/styles/narrow_carousel_large/public/content-carousel-images/07979.jpg?itok=m-MumPZY',
    uid: 3,
    archived: false
}, {
    name: 'Hawaii Landscape',
    city: 'Landscape City',
    state: 'Landscape State',
    cuisines: 'BeeseChurgerScape',
    pic: 'https://www.gohawaii.com/sites/default/files/styles/narrow_carousel_large/public/content-carousel-images/Molokai%20Kalaupapa%20Coast%20aerial.jpg?itok=U9LXGaFx',
    uid: 4,
    archived: false
}])
.then(() => {
    console.log('Successful Seed creation');
    process.exit();
})
.catch(err => {
    console.log('Unsuccessful Seed creation=>', err);
    process.exit();
});
